...
sub printTitles {
    my ($sessionType) = @_;
    if ($sessionType =~ "EXP") {
        print "SessionID, Time, Event, Round, Actor, Action, Quant, Price, ";
        print "Buyer, Seller, BestBid, BestAsk, Role, ";
        print "Actual, Target, Outcome, ";
        print "Hint, Pos. Targets, Pos. Clues\n";
    } else {
        print "SessionID, Time, Event, Actor, Action, Quant, End Price, Buyer, Seller, ",
                 "BestBid, BestAsk, start price, market, outcome, goalAction, ",
                 "goalIssue, costLimit, priceTarget, totalCost\n";
    }
}
...
#print ".$outcome\n";
    if ($sessionType =~ "EXP") {
        printExp("", $time, $event, $actor, $action, $quant, $price, $buyer, $seller, $type, $state, $clue, $outcome);
    } else {
        printPM("", $time, $event, $actor, $action, $quant, $price, $buyer, $seller, $type, $state,
            $market, $outcome, $goalAction, $goalIssue, $costLimit, $priceTarget, $totalCost);
    }
}

sub printExp {
        my ($filename, $time, $event, $actor, $action, $quant, $price, $buyer, $seller, $type, $state, $clue, $outcome) = @_;
        my $role = $roles{$actor};
        my $actual = $actualValues[$round - 1];
        my $target = getTarget($actor, $round);
        my $hint = getHint($actor, $round);
        my $posTargets = 0 + $posTargets[$round - 1];
        my $posClues = 0 + $posClues[$round - 1];

        print "$filename, $time, $event, $round, $actor, $action, $quant, $price, ";
        print "$buyer, $seller, $bestBid, $bestAsk, $role, ";
        print "$actual, $target, $outcome, ";
        print "$hint, $posTargets, $posClues\n";
}

sub printPM {
        my ($filename, $time, $event, $actor, $action, $quant, $price, $buyer, $seller, $type, $state, $market, $outcome, $goalAction, $goalIssue, $costLimit, $priceTarget, $totalCost) = @_;

        my ($shortname) = ($filename =~ /.*\/([^\/]+)/ );
        my $mmPrice = mmPrice($market, $outcome);

        print "$shortname, $time, $event, $actor, $action, $quant, $price, $buyer, $seller, $bestBid, $bestAsk, $mmPrice, $market, $outcome, $goalAction, $goalIssue, $costLimit, $priceTarget, $totalCost \n";
}
